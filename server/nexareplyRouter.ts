...
export const nexareplyRouter = router({
  ...
  workspace: router({
    ...
    products: router({
      ...
      listPage: protectedProcedure.input(organizationInput.extend({ query: z.string().max(160).optional(), includeArchived: z.boolean().optional(), cursor: z.object({ brand: z.string().max(100), model: z.string().max(160), id: z.number().int().positive() }).optional(), limit: z.number().int().min(10).max(50).default(20) })).query(async ({ ctx, input }) => nexareplyRepository.listProductPage(await workspaceScope(ctx.user.id, input.organizationId), input)),
