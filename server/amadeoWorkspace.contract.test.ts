...
describe("Amadeo workspace response contracts", () => {
  it("returns secret-safe knowledge, inbox, alerts, ticket and analytics DTOs", async () => {
    ...
    vi.spyOn(nexareplyRepository, "listMessages").mockResolvedValue([{ id: 5, conversationId: 3, sender: "customer", body: "ფასი?", source: "meta", isDraft: false, automated: false, deliveryStatus: "received", approvedAt: null, createdAt: new Date(), inboundEventId: "not-public", automationEventId: "not-public" }] as never);
    ...
    expect(messages[0]).toMatchObject({ deliveryStatus: "received", source: "meta", automated: false });
    ...
    expect(JSON.stringify({ knowledge, conversations, messages, alerts, tickets, analytics })).not.toMatch(/access.?token|app.?secret|provider.?token|dedupe.?key|inbound.?event|automation.?event/i);
  });

  it("supports assistant/knowledge/ticket actions and persists sent or failed inbox delivery states", async () => {
    const api = await caller();
    vi.spyOn(nexareplyRepository, "getOrganization").mockResolvedValue({ aiPersona: "Amadeo სუნამოების კონსულტანტი", aiTone: "თბილი", replyLength: "normal", fallbackMessage: "გადავამოწმებ", autoReplyEnabled: true, accessToken: "never-return" } as never);
    vi.spyOn(nexareplyRepository, "updateAssistantSettings").mockResolvedValue({ aiPersona: "ზუსტი კონსულტანტი", aiTone: "მშვიდი", replyLength: "short", fallbackMessage: "ticket გაიხსნება", autoReplyEnabled: false, appSecret: "never-return" } as never);
    ...
    await expect(api.nexareply.workspace.assistant.settings({ organizationId: scope.organizationId })).resolves.toEqual({ aiPersona: "Amadeo სუნამოების კონსულტანტი", aiTone: "თბილი", replyLength: "normal", fallbackMessage: "გადავამოწმებ", autoReplyEnabled: true });
    await expect(api.nexareply.workspace.assistant.update({ organizationId: scope.organizationId, aiPersona: "ზუსტი კონსულტანტი", aiTone: "მშვიდი", replyLength: "short", fallbackMessage: "ticket გაიხსნება", autoReplyEnabled: false })).resolves.toEqual({ aiPersona: "ზუსტი კონსულტანტი", aiTone: "მშვიდი", replyLength: "short", fallbackMessage: "ticket გაიხსნება", autoReplyEnabled: false });
