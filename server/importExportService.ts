...
export type CatalogMapping = { brand: string; model: string; sku: string; priceGel: string; stock: string; color: string; storage?: string; installment?: string; warranty?: string };
export type ValidCatalogRow = { brand: string; model: string; sku: string; priceGel: string; stock: number; color: string; storage: string; installment: string; warranty: string };

const defaultMapping: CatalogMapping = { brand: "ბრენდი", model: "მოდელი", sku: "SKU", priceGel: "ფასი GEL", stock: "მარაგი", color: "ფერი", storage: "მეხსიერება", installment: "განვადება", warranty: "გარანტია" };
...
export function parseCatalogFile(base64: string, fileName: string, mapping: CatalogMapping = defaultMapping) {
  ...
  rawRows.forEach((raw, index) => {
    ...
    const model = readCell(raw, mapping.model);
    ...
    const missing = [!brand && "ბრენდი", !model && "მოდელი", !sku && "SKU", !priceInput && "ფასი GEL", !stockInput && "მარაგი"].filter(Boolean);
    ...
    validRows.push({
      brand, model, sku, priceGel: price.toFixed(2), stock,
      color: readCell(raw, mapping.color) || "—",
      storage: readCell(raw, mapping.storage) || "—",
      installment: readCell(raw, mapping.installment) || "არ არის მითითებული",
      warranty: readCell(raw, mapping.warranty) || "არ არის მითითებული",
    });
  });
...
export async function commitCatalogImport(scope: WorkspaceScope, input: { base64: string; fileName: string; mapping?: CatalogMapping }) {
  ...
  for (const row of parsed.validRows) {
    try {
      ...
      commitErrors.push({ row: imported + commitErrors.length + 2, message: error instanceof Error ? error.message : "მონაცემის შენახვა ვერ მოხერხდა" });
    }
  }
...
export async function exportSalesCsv(scope: WorkspaceScope, kind: "leads" | "orders") {
  if (kind === "leads") {
