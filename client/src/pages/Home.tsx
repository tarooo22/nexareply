    ...
    <div className="nr-marketing min-h-screen overflow-x-hidden text-white">
      ...
      <main>
        <section className="nr-hero relative isolate overflow-hidden"><div className="nr-grid-overlay" /><div className="nr-glow nr-glow-a" /><div className="nr-glow nr-glow-b" /><div className="container nr-hero-grid">
          <div className="max-w-2xl"><span className="nr-eyebrow"><Sparkles className="size-3.5" />AI გაყიდვების სივრცე ქართული ბიზნესისთვის</span><h1 className="nr-hero-title">AI, რომელიც შენს Messenger-ში <span>ყიდის და პასუხობს</span></h1><p className="nr-hero-lead">დააკავშირე Facebook გვერდი, ატვირთე პროდუქტები და ბიზნეს-ცოდნა — NexaReply ავტომატურად უპასუხებს მომხმარებელს შენს მონაცემებზე დაყრდნობით. კონტროლი კი ყოველთვის შენ გრჩება.</p>
<div className="flex flex-wrap gap-3"><Link href="/auth" className="nr-btn nr-btn-primary">დაიწყე უფასოდ <ArrowRight className="size-4" /></Link><Link href="/demo" className="nr-btn nr-btn-ghost">ნახე Demo</Link>
</div><div className="nr-micro"><span>ბარათი არ სჭირდება</span><b /> <span>5 წუთში დაყენება</span><b /> <span>ნებისმიერ დროს გააჩერე</span></div>
</div>
          <ChatPreview />
        </div></section>
        <div className="nr-trust"><div className="container nr-trust-grid">{[[Zap, "სწრაფი დაკავშირება"], [Clock3, "24/7 AI პასუხები"], [ShieldCheck, "ცოდნაზე დაფუძნებული"], [UsersRound, "ადამიანზე გადართვა"], [MessageSquareText, "ერთიანი Inbox"]].map(([Icon, label]) => <div key={label as string}><Icon className="size-4" />{label as string}</div>)}</div></div>
