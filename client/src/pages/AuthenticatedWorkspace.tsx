...
import { ArrowRight, Building2, ChevronDown, Loader2, LockKeyhole, Menu, Monitor, Moon, Plus, ShieldCheck, Sun, UserPlus, UsersRound } from "lucide-react";
...
function PanelLoading({ title }: { title: string }) {
  return <div className="mt-5 flex min-h-32 items-center justify-center gap-3 rounded-2xl border border-dashed border-border bg-card/50 text-sm font-bold text-muted-foreground"><Loader2 className="size-4 animate-spin text-primary" />{title}</div>;
}
...
export default function AuthenticatedWorkspace() {
  ...
  const screen = useMemo(() => {
    ...
    if (currentSection === "inbox") return <WorkspaceInboxScreen {...props} />;
