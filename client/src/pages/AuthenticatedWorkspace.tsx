...
export default function AuthenticatedWorkspace() {
  ...
  return <main className="min-h-screen bg-background text-foreground lg:flex"><aside className="flex w-full flex-col border-b border-sidebar-border bg-sidebar p-4 lg:min-h-screen lg:w-72 lg:border-b-0 lg:border-r"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-primary text-primary-foreground"><Building2 className="size-5" /></span><div><p className="text-sm font-black">NexaReply · Amadeo</p><p className="text-xs text-muted-foreground">persistent sales workspace</p></div></div><label className="mt-6 block text-xs font-bold text-muted-foreground">ორგანიზაცია<div className="relative mt-1"><select value={organization?.id ?? ""} onChange={(event) => { setOrganizationId(Number(event.target.value)); setSection("overview"); }} className="h-11 w-full appearance-none rounded-xl border border-sidebar-border bg-sidebar-accent/60 px-3 pr-9 text-sm font-bold outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring">{organizations.data?.map(({ organization: item, membership }) => <option key={item.id} value={item.id}>{item.name} · {membership.role}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 size-4 text-muted-foreground" /></div></label><WorkspaceNav role={role} active={currentSection} onSelect={setSection} /><Link href="/demo" className="mt-6 inline-flex min-h-10 items-center gap-2 rounded-xl border border-sidebar-border bg-sidebar-accent/40 px-3 text-sm font-bold hover:bg-sidebar-accent">Public Demo Mode <ArrowRight className="size-4" /></Link></aside><section className="min-w-0 flex-1 p-5 md:p-8"><div className="mx-auto max-w-7xl"><header className="flex flex-col justify-between gap-4 rounded-3xl border border-border bg-card p-6 shadow-sm md:flex-row md:items-center"><div><p className="text-sm font-bold text-primary">Authenticated Amadeo workspace</p><h1 className="mt-1 text-2xl font-black">{activeNav?.label ?? "მიმოხილვა"}</h1><p className="mt-2 text-sm text-muted-foreground">{organization?.name ?? "Workspace"} · {role === "owner" ? "ორგანიზაციის მფლობელი" : "გაყიდვების ოპერატორი"}</p></div><span className="inline-flex w-fit items-center gap-2 rounded-xl bg-secondary px-3 py-2 text-sm font-bold"><ShieldCheck className="size-4" />{role === "owner" ? "owner controls" : "operator scope"}</span></header>{screen}</div></section></main>;
}
