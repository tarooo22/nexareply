...
export default function AuthenticatedWorkspace() {
  ...
  const screen = useMemo(() => {
    ...
    return <AmadeoOverviewScreen {...props} onNavigate={setSection} />;
  ...
  const organizationPicker = <label className="mt-6 block text-xs font-bold text-sidebar-foreground/60">ორგანიზაცია<div className="relative mt-1"><select value={organization?.id ?? ""} onChange={(event) => { setOrganizationId(Number(event.target.value)); setSection("overview"); setMobileNavigationOpen(false); }} className="h-11 w-full appearance-none rounded-xl border border-sidebar-border bg-sidebar-accent/60 px-3 pr-9 text-sm font-bold outline-none focus-visible:ring-2 focus-visible:ring-sidebar-ring">{organizations.data?.map(({ organization: item, membership }) => <option key={item.id} value={item.id}>{item.name} · {membership.role}</option>)}</select><ChevronDown className="pointer-events-none absolute right-3 top-3.5 size-4 text-sidebar-foreground/60" /></div></label>;
  ...
  return <main className="min-h-screen bg-[radial-gradient(circle_at_top_right,_rgba(124,58,237,0.10),_transparent_28%),hsl(var(--background))] text-foreground lg:flex"><aside className="hidden w-72 shrink-0 flex-col border-r border-sidebar-border bg-sidebar px-4 py-5 lg:flex"><div className="flex items-center gap-3"><span className="grid size-10 place-items-center rounded-xl bg-primary text-primary-foreground"><Building2 className="size-5" /></span><div><p className="text-sm font-black">NexaReply · Amadeo</p><p className="text-xs text-sidebar-foreground/55">გაყიდვების ოპერაციები</p></div></div>{organizationPicker}{navigation}</aside><section className="min-w-0 flex-1"><header className="sticky top-0 z-20 border-b border-border/80 bg-background/90 px-4 py-3 backdrop-blur supports-[backdrop-filter]:bg-background/75 lg:hidden"><div className="flex items-center justify-between gap-3"><div className="flex min-w-0 items-center gap-2"><span className="grid size-9 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground"><Building2 className="size-4" /></span><div className="min-w-0"><p className="truncate text-sm font-black">NexaReply · Amadeo</p><p className="truncate text-xs text-muted-foreground">{activeNav?.label ?? "მიმოხილვა"}</p></div></div><Sheet open={mobileNavigationOpen} onOpenChange={setMobileNavigationOpen}><SheetTrigger asChild><button type="button" className="inline-grid size-11 place-items-center rounded-xl border border-border bg-card focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" aria-label="Workspace მენიუს გახსნა"><Menu className="size-5" /></button></SheetTrigger><SheetContent side="left" className="w-[86vw] max-w-sm overflow-y-auto border-sidebar-border bg-sidebar p-4"><SheetHeader className="px-0 pt-1"><SheetTitle className="flex items-center gap-2"><span className="grid size-9 place-items-center rounded-xl bg-primary text-primary-foreground"><Building2 className="size-4" /></span>NexaReply</SheetTitle><SheetDescription>Amadeo გაყიდვების სამუშაო სივრცე</SheetDescription></SheetHeader>{organizationPicker}{navigation}</SheetContent></Sheet></div></header><div className="p-4 sm:p-6 lg:p-8"><div className="mx-auto max-w-7xl"><header className="flex flex-col justify-between gap-4 rounded-2xl border border-border bg-card p-5 shadow-sm md:flex-row md:items-center md:p-6"><div><p className="text-sm font-bold text-primary">Amadeo-ის სამუშაო სივრცე</p><h1 className="mt-1 text-2xl font-black tracking-tight">{activeNav?.label ?? "მიმოხილვა"}</h1><p className="mt-2 text-sm text-muted-foreground">{organization?.name ?? "Workspace"} · {role === "owner" ? "ორგანიზაციის მფლობელი" : "გაყიდვების ოპერატორი"}</p></div><span className="inline-flex w-fit items-center gap-2 rounded-xl bg-secondary px-3 py-2 text-sm font-bold"><ShieldCheck className="size-4 text-primary" />{role === "owner" ? "მფლობელის წვდომა" : "ოპერატორის წვდომა"}</span></header>{screen}</div></div></section></main>;
}
