...
- [ ] Design a minimal-permission Facebook Login for Business configuration strategy, including config_id handling and a decision on retaining the System User fallback.
...
- [ ] Redesign the Georgian Meta connection wizard for a single primary Facebook connection path, compact page selection/auto-selection, success and recoverable error states.
- [ ] Expand security, tenant-isolation, OAuth, webhook, connection, manual-gating, and responsive UI regression coverage for the onboarding flow.
- [ ] Reconcile META_SETUP.md and create META_APP_REVIEW.md with exact permissions, reviewer flow, screencast guidance, current limitations, and owner-side approval steps.
