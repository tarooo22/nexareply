...
- [x] Audit the Meta App Facebook Login availability state that blocks non-owner accounts and identify the exact dashboard-side prerequisite.
- [x] Remediate or document the necessary Meta App roles, live-mode/review, and required app-detail configuration without changing Amadeo credentials.
- [ ] Retest a non-owner Facebook OAuth entry safely and document the rollout status for self-service Page connections.
- [x] Audit Meta Dashboard App Mode, Facebook Login configuration, roles, review and data-use requirements for the current public OAuth blocker.
- [x] Complete or explicitly document all Meta-side public rollout prerequisites without rotating Amadeo credentials or changing existing Page delivery.
...
- [x] Implement a verified account-owner data-deletion request workflow and deletion audit trail to support the public Meta policy instructions without exposing tenant data.
...
- [ ] Confirm whether the current server-side Meta App credentials match the new Automated Messenger App that issued the Page Access Token.
...
## Button functionality audit — current request
...
- [x] Repair only confirmed catalog/asset/import-export gaps while retaining tenant-scoped storage keys, server-side validation and owner/operator authorization.
