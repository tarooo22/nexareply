...
`background_jobs` persists conversation processing intent and idempotency keys. The current autoscaling request runtime does **not** provide a continuously running worker, so a saved ten-second debounce timestamp is not a production execution guarantee. A protected server-side trigger is available at `POST /api/internal/worker/process-conversations`; deploy a durable scheduler/worker and configure its independent `WORKER_TRIGGER_SECRET` through managed server secrets before claiming a 10-second automation SLA. See [docs/durable_worker_trigger.md](docs/durable_worker_trigger.md).
