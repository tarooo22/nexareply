...
რეალურ webhook და queue workload-ს დასჭირდება გარედან მიღებული event-ის სწრაფი acknowledgment და გამძლე, background დამუშავება. Demo Mode-ს ეს არ სჭირდება; ამიტომ პირველ ეტაპზე debounce იმიტირდება client/server demo service-ში და ინახავს state-ს. რეალური Meta adapter-ის წინ საჭიროა ოფიციალური webhook შესაძლებლობის ხელახლა გადამოწმება და hosting გადაწყვეტილება: stateless handler + durable queue, ან მუდმივი background worker, თუ მოთხოვნის latency ამას მოითხოვს. არ უნდა გამოყენებულ იქნას წუთობრივად გაშვებული polling.
